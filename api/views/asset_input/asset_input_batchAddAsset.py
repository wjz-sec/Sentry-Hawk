...
from app.models import Asset_scan_input, Projects, Tags
...
class AssetBatchAddForm(BaseAssetBatchAddForm):
    def clean_asset(self):
        ...
        if asset_project:
            ...
            if Asset_scan_input.objects.filter(asset=asset, asset_project=asset_project).exists():
...
class BatchAddAsset(View):
    def post(self, request):
        ...
        try:
            ...
            # 批量创建资产
            Asset_scan_input.objects.bulk_create(
                [Asset_scan_input(**data) for data in assets_data]
            )
