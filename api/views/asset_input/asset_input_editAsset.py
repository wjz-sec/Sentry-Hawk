...
class editAsset(View):
    def post(self, request):
        ...
        duplicate_check = Asset_scan_input.objects.filter(
            asset=data['asset'],
        ...
        try:
            ...
            try:
                ...
                # 先只用名称查找，看看是否存在
                all_assets = Asset_scan_input.objects.filter(asset=asset_name)
            ...
            # 只有在有字段更新时才执行更新操作
            if update_fields:
                Asset_scan_input.objects.filter(asset_id=asset_id).update(**update_fields)
                res['code'] = 200
                res['msg'] = "更新成功"
            else:
                res['msg'] = "没有需要更新的字段"

        except Asset_scan_input.DoesNotExist:
